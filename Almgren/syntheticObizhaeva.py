...
TOTAL_SHARES1 = 500000                                               # Total number of shares to sell                                              # Total number of shares to sell
...
class MarketEnvironment():
    ...
    def reset(self, seed = 0, liquid_time = LIQUIDATION_TIME, num_trades = NUM_N, lamb1 = LLAMBDA1):
        
        # Initialize the environment with the given parameters
        self.__init__(randomSeed = seed, lqd_time = liquid_time, num_tr = num_trades, lambd1 = lamb1)
        
        # Set the initial state to [0,0,0,0,0,0,1,1]
        self.initial_state = np.array([np.log(F_0), self.total_shares1, self.num_n + 2]])
    ...
    def step(self, action1):
        ...
        info.done1 = False
        self.prices = np.zeros((self.num_n+1))
        self.actions = np.zeros(self.num_n +2)
...
#             sharesToSellNow = min(self.shares_remaining * action, self.shares_remaining)
        ...
        if self.transacting1:
            ...
            info.currentPermanentImpact = self.permanentImpact(info.share_to_sell_now1)
            info.currentTemporaryImpact = self.temporaryImpact(info.share_to_sell_now1)
            ...
            if self.prevUtility1 == 0:
                ...
                reward1 = info.share_to_sell_now1 * (info.exec_price - self.startingPrice)
        ...
        return (state, reward1, info.done1, info)
    ...
    def get_AC_expected_shortfall(self, sharesToSell,kappa):
        ...
        nft = np.nan_to_num(np.tanh(0.5 * kappa * self.tau) * (self.tau * np.sinh(2 * kappa * self.liquidation_time)) \
                                                      + 2 * self.liquidation_time * np.sinh(kappa * self.tau))    
        dft = np.nan_to_num(2 * (self.tau ** 2) * (np.sinh(kappa * self.liquidation_time) ** 2))
        fot = nft / dft       
        return ft + st + (tt * fot)  
    ...
    def get_AC_variance(self, sharesToSell,kappa):
        ...
        nst  = np.nan_to_num(self.tau * np.sinh(kappa * self.liquidation_time) * np.nan_to_num(np.cosh(kappa * (self.liquidation_time - self.tau))) \
    ...
    def get_trade_list(self,kappa):
        ...
        ftd = np.nan_to_num(np.sinh(kappa * self.liquidation_time))
    ...
    def get_random_walk(scale_param = 5, F_0=1000, N):
        P = self.F_0
        F = [P]
